...
class Customer(models.Model):
    ...
    """
    qq = models.CharField(
        'QQ', max_length=64, unique=True, help_text='QQ号必须唯一')
    qq_name = models.CharField('QQ昵称', max_length=64, blank=True, null=True)
    name = models.CharField(
        '姓名', max_length=32, blank=True, null=True, help_text='学员报名后，请改为真实姓名')
    ...
    birthday = models.DateField(
        '出生日期', default=None, help_text="格式yyyy-mm-dd", blank=True, null=True)
    phone = models.BigIntegerField('手机号', blank=True, null=True)
    source = models.CharField(
        '客户来源', max_length=64, choices=source_type, default='qq')
    ...
    class_type = models.CharField(
        "班级类型", max_length=64, choices=class_type_choices, default='fulltime')
    ...
    def show_status(self):
        ...
        return mark_safe(
            '<span style="background-color: {};color: white">{}</span>'.format(
                _status_color[self.status], self.get_status_display()))
...
class ClassList(models.Model):
    ...
    contract = models.ForeignKey(
        'ContractTemplate', verbose_name="选择合同模版", blank=True, null=True)
    ...
    def show_campuses(self):
        return self.campuses.name

    def show_teacher(self):
        return '|'.join(
            [i['name'] for i in self.teachers.all().values('name')])
...
class ConsultRecord(models.Model):
    ...
    status = models.CharField(
        ...
        help_text="选择客户此时的状态")
    consultant = models.ForeignKey(
        "UserProfile", verbose_name="跟进人", related_name='records')
...
class Enrollment(models.Model):
    ...
    why_us = models.TextField(
        "为什么报名", max_length=1024, default=None, blank=True, null=True)
...
class PaymentRecord(models.Model):
    ...
    enrolment_class = models.ForeignKey(
        'ClassList', verbose_name='所报班级', blank=True, null=True)
    ...
    status = models.IntegerField(
        verbose_name='审核', default=1, choices=status_choices)
...
class UserProfile(AbstractBaseUser, PermissionsMixin):
    ...
    department = models.ForeignKey(
        'Department', default=None, blank=True, null=True)
    mobile = models.CharField(
        '手机', max_length=32, default=None, blank=True, null=True)
